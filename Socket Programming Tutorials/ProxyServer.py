...
while True:    
    ...
    if(request != b''):
        ...
        try:
            file = open(url.decode() + '_' + extra_info.decode() + ".html", 'r') #If file exists, load it up (cache)
            content = file.read() 
            file.close()
            content = content.split('<!doctype html>')[1] 
            connectionSocket.send(('HTTP/1.1 200 OK\n\n' + content).encode()) #Content should show up here.
            ...
            clientSocket.settimeout(5) #Timeout = 5sec.
            ...
            if(code.decode() == '200 OK'): #If webpage opens and is not a redirect, cache the contents. 
                f = open(url.decode() + '_' + extra_info.decode() + '.html', "wb") #Unique identifier for every page.
